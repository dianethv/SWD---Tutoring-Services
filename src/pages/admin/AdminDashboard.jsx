...
export default function AdminDashboard() {
    ...
    const opsBasePath = isAdmin ? '/admin' : '/tutor';

    const getDisplayName = () => {
        const parts = currentUser?.name?.split(' ') || [];
        return parts.length >= 3 ? `${parts[0]} ${parts[parts.length - 1]}` : currentUser?.name || 'Team Member';
    };
    ...
    const tipRows = isAdmin
        ...
        : [
            ['Serve the next student quickly', 'Keep the line moving so estimated waits stay realistic.'],
            ['Use no-show carefully', 'Mark absences accurately so queue history remains trustworthy.'],
            ['Watch the live load', 'Keep an eye on the busiest service and be ready for spikes.'],
        ];
    ...
    return (
        <div className="admin-dashboard-page">
            <section className="tc-page-hero">
                ...
                >
                    <div>
                        <span className="tc-page-eyebrow">{isAdmin ? 'Admin Console' : 'Tutor Console'} · Live</span>
                        ...
                        <p className="tc-page-sub">
                            {activeStudents > 0
                                ...
                                : isAdmin
                                    ...
                                    : 'All queues are clear. A good time to prep for the next tutoring session.'}
                        </p>
                        <div style={{ marginTop: 24, display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                            <Link
                                to={`${opsBasePath}/queues`}
                                style={{
                                    padding: '12px 22px',
                                    borderRadius: 12,
                                    background: '#C8102E',
            ...
            <section className="tc-tip">
                <span className="tc-page-eyebrow">{isAdmin ? 'Management Tips' : 'Tutor Tips'}</span>
                <h3 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 18, fontWeight: 700, color: '#1c1917', margin: '12px 0 14px 0', letterSpacing: '-0.012em' }}>
                    {isAdmin ? 'Run a healthy tutoring center' : 'Keep the queue moving smoothly'}
                </h3>
                <ul style={{ margin: 0, padding: 0, listStyle: 'none' }}>
                    {tipRows.map(([title, body], i) => (
                        <li
                            key={title}
                            style={{
                                display: 'flex',
