...
export default function JoinQueue() {
    ...
    return (
        <div className="animate-fade-in-up">
            ...
            {joinError && (
                <div className="fixed top-20 right-4 z-50 p-4 rounded-2xl bg-red-50 border border-red-200 text-red-700 text-sm font-medium shadow-lg toast-enter flex items-center gap-2">
                        ...
                        >
                            <div className="p-5">
                                ...
                                <div className="flex items-center gap-4 mb-4 text-xs text-stone-500">
                                    ...
                                    <span className={`px-2 py-0.5 rounded-md text-xs font-medium priority-${service.priorityLevel}`}>
                                ...
                                <div className="flex items-center justify-between p-3 rounded-xl bg-stone-50 mb-4">
                                    <div>
                                        <p className="text-xs text-stone-500">Estimated wait</p>
                                        <p className="font-bold text-stone-800">{eta > 0 ? `~${eta} min` : 'No wait!'}</p>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-xs text-stone-500">Your position</p>
                                        <p className="font-bold text-stone-800">
                                            {isInQueue ? `#${userEntry.position}` : `#${queueLength + 1}`}
                                        </p>
                                    </div>
                                </div>

                                {/* Expand section for notes when selected */}
                                ...
                                {service.isOpen ? (
                                    isInQueue ? (
                                        ...
                                        <button
                                            ...
                                            className="w-full py-2.5 rounded-xl text-white text-sm font-semibold transition-all hover:opacity-90 cursor-pointer border-none"
                                            style={{ background: 'linear-gradient(135deg, #C8102E, #E8384F)' }}
                                            id={`join-queue-${service.id}`}
                                        >
                                            Join Queue
                                        </button>
                                    ) : (
